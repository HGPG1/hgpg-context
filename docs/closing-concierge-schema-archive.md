...
Original domain: concierge.homegrownpropertygroup.com (DNS removed 2026-05-05)
Original repo: HGPG1/closing-concierge

Useful as a reference if the TM-integrated concierge ever needs to be enriched with smart-device, utility, or move-in/move-out detail capture.
...
### sms_schedule
Scheduled outbound SMS to seller/buyer
cat > docs/closing-concierge-schema-archive.md <<'EOF'
# Closing Concierge (standalone) — schema archive
...
- id (pk), property_id (fk), recipient_phone, recipient_type, message, scheduled_date, sent_at, sent, error

### email_log
Sent-email audit.
- id (pk), property_id (fk), recipient_email, recipient_type, subject, status, sent_at
